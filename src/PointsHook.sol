...
contract PointsHook is BaseHook, ERC20 {
    ...
    function afterSwap(
        ...
        // We check if the token pair is ETH-TOKEN
        if (!key.currency0.isAddressZero()) return (this.afterSwap.selector, 0);

        // We are only minting if user is buying TOKEN with ETH
        if (!swapParams.zeroForOne) return (this.afterSwap.selector, 0);

        // We have to mint 20% of the amount of ETH spent
    ...
    function _assignPoints(bytes calldata hookData, uint256 points) internal {
        // If no hookdata is passed, no points will be assigned
        if (hookData.length == 0) return;

        // Extract user from hookData
